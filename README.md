...
<p align="center">
  <strong>Current version: 5.9.0</strong>
</p>
...
The main application is contained in [`index.html`](index.html).
...
English and Swedish are supported. **Changing language does not reload phone or connector defaults and does not modify any measurements or selections.** The currently entered values remain in place; only interface text, source labels and preview/status text are refreshed.
...
- **Minimalist** — tapered central spine with two mirrored side clamps aligned directly with the cross-bridge, plus a centred lower support
...
The holder supports several ways to retain a USB-C or Lightning charging connector:

- **Embedded bottom dock with locking screw** — the default charging-retention option. A rectangular connector pocket is built through the lower holder construction and shifted 1.0 mm forward from the rear wall for improved alignment with the phone charging port. The dock is physically fused to the holder by overlapping both lower supports into its side walls and by a rear attachment web into the holder spine/back. The connector is inserted from underneath and a small front locking screw presses against the plastic connector housing to keep it stationary. The screw should never bear against the electrical/metal connector itself.
- **Two-piece screw-on L-shaped cable clamp** — the connector sits in an open cradle. The separately printed L-cover has a lower tongue that slides underneath the cable mount and a solid front plate secured with two screws. The former U-shaped cable cut-out in the front plate has been removed; the cable passage remains only in the under-tab where it is needed.

The L-cover automatically compensates its screw-hole positions for the under-tab thickness. M3 hardware is a practical starting point for the screw-on options, but always measure the actual cable housing and verify screw length before assembly.
...
- Locking-screw pilot-hole diameter for the embedded bottom dock
- Screw-on cover thickness
- Cover screw clearance-hole diameter
- Holder pilot-hole diameter
- L-cover under-tab length
...
The complete generator setup can be exported to a portable **JSON configuration file** and imported again later.

The configuration includes:

- Phone model selection and manually edited phone dimensions
- Case allowances and fit clearances
- Holder design and all geometry measurements
- Button/camera settings
- Charging mode, connector dimensions and screw settings
- Back-mount/Brodit/AMPS settings
- Fit-test settings
- Preview checkboxes and interface language

Import restores the saved values directly and does **not** reapply phone or connector presets over the imported measurements. The file includes a configuration-format version and generator version so future releases can handle compatibility cleanly.
...
- **Brodit ball joint – 3-hole vertical mounting pattern**, using a default 20.5 mm centre-to-centre spacing between adjacent holes (41.0 mm outer-to-outer), with the centre hole automatically 2 mm larger than the upper and lower holes
...
Hole spacing, hole diameter, recesses, orientation and position can be adjusted manually. The Brodit ball-joint spacing defaults to 20.5 mm centre-to-centre between adjacent holes.
